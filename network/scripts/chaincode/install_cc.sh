...
# ===========================
docker exec "$CLI_NAME" peer chaincode instantiate -o "$ORDERER_NAME":7050 -C "$CHANNEL_NAME" -n "$CC_NAME" -l ${LANGUAGE} -v v0  -c '{"Args":[]}' -P "OR('Org1MSP.member', 'Org2MSP.member' )" --tls true --cafile $ORDERER_CA
